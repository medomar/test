...
exports.updateById = (req,res) => {
    ...
    var value = req.body.value;
    var channelId = req.body.channelId;
    ArchiveManager.updateById(req.params.userId,key,value,channelId).then(archiveList => {
        res.send(archiveList);
    }).catch(err => {
...
ArchiveManager.deleteById(req.params.userId).then(archiveList => {
        res.send(archiveList);
    }).catch(err => {
        return res.status(500).send({
            message: "Error updating note with id " + req.params.userId
...
exports.findById = (req, res) => {
    ArchiveManager.findById(req.params.userId).then(archiveList => {
        res.send(archiveList);
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Some error occurred while retrieving Archive."
